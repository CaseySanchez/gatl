...
namespace ga {
    ...
    template<typename BaseCoefficientType, typename BaseCoefficient>
    constexpr decltype(auto) pow(scalar_clifford_expression<BaseCoefficientType, BaseCoefficient> const &base, NonCliffordExpressionType auto const &exponent) {
        return pow(base, scalar(exponent));
    }

    template<typename ExponentCoefficientType, typename ExponentCoefficient>
    constexpr decltype(auto) pow(NonCliffordExpressionType auto const &base, scalar_clifford_expression<ExponentCoefficientType, ExponentCoefficient> const &exponent) {
