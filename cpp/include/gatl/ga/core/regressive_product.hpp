...
namespace ga {
    ...
    constexpr decltype(auto) rp(clifford_expression<LeftCoefficientType, LeftExpression> const &lhs, NonCliffordExpressionType auto const &rhs, metric_space<MetricSpaceType> const &mtr) GA_NOEXCEPT {
        return rp(lhs, scalar(rhs), mtr);
    }

    template<typename RightCoefficientType, typename RightExpression, typename MetricSpaceType>
    constexpr decltype(auto) rp(NonCliffordExpressionType auto const &lhs, clifford_expression<RightCoefficientType, RightExpression> const &rhs, metric_space<MetricSpaceType> const &mtr) GA_NOEXCEPT {
        return rp(scalar(lhs), rhs, mtr);
    }

    template<typename MetricSpaceType>
    constexpr decltype(auto) rp(NonCliffordExpressionType auto const &lhs, NonCliffordExpressionType auto const &rhs, metric_space<MetricSpaceType> const &mtr) GA_NOEXCEPT {
