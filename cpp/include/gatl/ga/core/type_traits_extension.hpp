...
namespace ga {
    ...
    namespace detail {
        ...
        GA_ALWAYS_INLINE GA_HOST_DEVICE constexpr decltype(auto) _to_list_initialization(Tuple &&tuple, std::index_sequence<Indices...>) GA_NOEXCEPT {
            return { std::get<Indices>(std::move(tuple))... };
        }

        template<typename Tuple>
        GA_ALWAYS_INLINE GA_HOST_DEVICE constexpr decltype(auto) to_list_initialization(Tuple &&tuple) GA_NOEXCEPT {
