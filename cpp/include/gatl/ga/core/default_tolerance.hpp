...
namespace ga {
    ...
    GA_HOST_DEVICE constexpr decltype(auto) default_tolerance() GA_NOEXCEPT {
        return c<0, ValueType>;
    }

    template<>
    GA_HOST_DEVICE constexpr decltype(auto) default_tolerance<std::float_t>() GA_NOEXCEPT {
        return scalar(GA_DEFAULT_FLT_TOLERANCE);
    }

    template<>
    GA_HOST_DEVICE constexpr decltype(auto) default_tolerance<std::double_t>() GA_NOEXCEPT {
