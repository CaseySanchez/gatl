...
namespace ga {
    ...
    namespace detail {
        ...
        template<typename CliffordExpression> requires (count_stored_values_v<typename CliffordExpression::expression_type> == 0)
        struct make_vector<CliffordExpression> {
            template<typename... Types>
    ...
    template<typename MetricSpaceType, typename IteratorType> requires (detail::is_iterator_v<IteratorType>)
