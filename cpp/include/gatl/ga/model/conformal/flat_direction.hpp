...
namespace ga {
    ...
    GA_HOST_DEVICE constexpr decltype(auto) dual_flat_direction(clifford_expression<CoefficientType, Expression> const &dual_flat, conformal_metric_space<D> const &mtr) GA_NOEXCEPT {
    ...
    template<typename CoefficientType, typename Expression, ndims_t D>
    GA_HOST_DEVICE constexpr decltype(auto) primal_flat_direction(clifford_expression<CoefficientType, Expression> const &primal_flat, conformal_metric_space<D> const &mtr) GA_NOEXCEPT {
        constexpr auto ni = e(c<D + 2>);
