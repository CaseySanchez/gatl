...
namespace ga {
    ...
    constexpr decltype(auto) unit(NonCliffordExpressionType auto const &arg, metric_space<MetricSpaceType> const &mtr) {
        return unit(scalar(arg), mtr);
    }

    constexpr decltype(auto) unit(NonCliffordExpressionType auto const &arg) {
