...
namespace ga {
    ...
    constexpr decltype(auto) rnorm_sqr(NonCliffordExpressionType auto const &arg, metric_space<MetricSpaceType> const &) GA_NOEXCEPT {
        return rnorm_sqr(scalar(arg), detail::real_metric_space());
    }

    constexpr decltype(auto) rnorm_sqr(NonCliffordExpressionType auto const &arg) GA_NOEXCEPT {
        return rnorm_sqr(scalar(arg), detail::real_metric_space());
    }
    ...
    template<typename MetricSpaceType>
    constexpr decltype(auto) rnorm(NonCliffordExpressionType auto const &arg, metric_space<MetricSpaceType> const &) {
        return rnorm(scalar(arg), detail::real_metric_space());
    }

    constexpr decltype(auto) rnorm(NonCliffordExpressionType auto const &arg) {
        return rnorm(scalar(arg), detail::real_metric_space());
    }
