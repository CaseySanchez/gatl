...
namespace ga {
    ...
    GA_HOST_DEVICE constexpr decltype(auto) take_grade(clifford_expression<CoefficientType, Expression> const &arg, scalar_clifford_expression<GradeCoefficientType, GradeCoefficient> const &k) GA_NOEXCEPT {
    ...
    GA_HOST_DEVICE constexpr decltype(auto) take_grade(Type const &arg, scalar_clifford_expression<GradeCoefficientType, GradeCoefficient> const &k) GA_NOEXCEPT {
        return take_grade(scalar(arg), k);
    }

    template<typename CoefficientType, typename Expression>
    GA_HOST_DEVICE constexpr decltype(auto) take_grade(clifford_expression<CoefficientType, Expression> const &arg, grade_t const k) GA_NOEXCEPT {
        return take_grade(arg, scalar(k));
    }

    template<typename Type>
    GA_HOST_DEVICE constexpr decltype(auto) take_grade(Type const &arg, grade_t const k) GA_NOEXCEPT {
    ...
    GA_HOST_DEVICE constexpr decltype(auto) take_largest_grade(clifford_expression<CoefficientType, Expression> const &arg, ToleranceType const &tol) GA_NOEXCEPT {
        auto [lazy, arg_, tol_] = make_lazy_context_tuple(arg, scalar(tol));
        return lazy.eval(take_grade(arg_, largest_grade(arg_, tol_)));
    }

    template<typename CoefficientType, typename Expression>
    GA_HOST_DEVICE constexpr decltype(auto) take_largest_grade(clifford_expression<CoefficientType, Expression> const &arg) GA_NOEXCEPT {
        return take_largest_grade(arg, default_tolerance<CoefficientType>());
    }

    template<typename Type, typename ToleranceType>
    GA_HOST_DEVICE constexpr decltype(auto) take_largest_grade(Type const &arg, ToleranceType const &tol) GA_NOEXCEPT {
        return take_largest_grade(scalar(arg), tol);
    }

    template<typename Type>
    GA_HOST_DEVICE constexpr decltype(auto) take_largest_grade(Type const &arg) GA_NOEXCEPT {
